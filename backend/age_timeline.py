...
from storage import AGE_CANDIDATES_DIR, AGE_DEBUG_DIR, AGE_PLAN_PATH, SOURCE_FACES_DIR
...
def _read() -> dict:
    if not AGE_PLAN_PATH.is_file():
        return {}
    try:
        value = json.loads(AGE_PLAN_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}
    return value if isinstance(value, dict) else {}


def _validation_by_name(age: int) -> dict[str, dict]:
    path = AGE_DEBUG_DIR / f"age{age:02d}_flux2_validation.json"
    if not path.is_file():
        return {}
    try:
...
def _candidate_files(age: int) -> list[dict]:
    prefix = f"age{age:02d}_"
    if not AGE_CANDIDATES_DIR.exists():
        return []
    validation = _validation_by_name(age)
    return [
        {
            "name": path.name,
            "url": f"/age-candidates/{path.name}",
            "size_kb": path.stat().st_size // 1024,
            "validation": validation.get(path.name),
        }
        for path in sorted(AGE_CANDIDATES_DIR.iterdir())
        if path.is_file()
...
def get_plan() -> dict:
    ...
    source = SOURCE_FACES_DIR / Path(str(current_photo or "")).name
    ...
    for stage in stages:
        if stage["kind"] == "current":
            ...
            stage["url"] = f"/identity-faces/{source.name}" if photo_exists else None
            stage["candidates"] = []
        else:
            candidates = _candidate_files(stage["age"])
...
def save_plan(
    ...
    *,
    ...
    if safe_name != current_photo or not (SOURCE_FACES_DIR / safe_name).is_file():
...
def _write(payload: dict) -> None:
    AGE_PLAN_PATH.parent.mkdir(parents=True, exist_ok=True)
    temporary = AGE_PLAN_PATH.with_suffix(".tmp")
    temporary.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    temporary.replace(AGE_PLAN_PATH)
...
def select_candidate(age: int, filename: str) -> dict:
    plan = get_plan()
...
def insert_visual_bridge_candidate(
    ...
    safe_name = Path(filename).name
    candidate_path = AGE_CANDIDATES_DIR / safe_name
    if (
        safe_name != filename
...
def remove_refinement(age: int) -> dict:
...
def refine_failed_segment(older_age: int, younger_age: int) -> dict:
