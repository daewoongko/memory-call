...
DIRECT_RISK_PATTERNS = [
    ("stroke_sign", "high", [
        r"갑자기[^.!?]{0,12}(말이 안 나오|말이 꼬이|(한쪽|왼쪽|오른쪽)\s*팔[^.!?]{0,8}(힘이 없|안 움직)|얼굴[^.!?]{0,8}(처지|비뚤))",
        r"(한쪽|왼쪽|오른쪽)\s*(팔|다리|얼굴)[^.!?]{0,10}(힘이\s*(하나도\s*)?없|안\s*움직|이상하게|저리|처지|비뚤)",
    ]),
    ("fall", "high", [
        r"넘어(?:졌|져서)[^.!?]{0,24}(일어나|움직이)[^.!?]{0,8}(힘들|못)",
        r"넘어(?:졌|져서)[^.!?]{0,20}(아프|아파|아팠|통증)",
    ]),
    ("breathing", "high", [r"숨(?:이)?[^.!?]{0,8}(차|안 쉬|막혀|못 쉬)"]),
    ("chest_pain", "high", [r"가슴[^.!?]{0,8}(아프|아파|아팠|통증|조여)"]),
    ("overdose", "high", [
        r"약[^.!?]{0,12}(두 번|세 번|여러 번|많이)[^.!?]{0,8}(먹|복용|드셨)",
    ]),
    ("self_harm", "high", [
        r"죽고 싶", r"살기 싫", r"없어지고 싶", r"사라지고 싶",
        r"없어지는\s*(게|것)[^.!?]{0,10}(나을|낫)",
        r"(살|사는\s*(게|것)|삶)[^.!?]{0,10}(의미가?\s*없|이유가?\s*없)",
        r"그만\s*살고\s*싶",
    ]),
