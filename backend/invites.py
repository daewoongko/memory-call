...
# 대웅의 승인된 성장 영상(726프레임, 30fps)과 정확히 같은 길이다.
INTRO_DURATION_SEC = 24.2
DEFAULT_RING_SEC = INTRO_DURATION_SEC

# 이전 코드와 기록 분석의 이름은 유지하되, 기기 유무와 상관없이 같은 24.2초를
...
def _now() -> str:
    # 24.2초 영상과 서버 마감을 맞추려면 초 단위 절삭으로 생기는 최대 1초의
    # 조기 만료를 피해야 한다.
...
def _decorate(row: dict) -> dict:
    ...
    # 위험 발화 역호출은 보호자가 즉시 받아야 하므로 가족 호출용 24.2초
    # 아바타 소개 구간을 적용하지 않는다.
