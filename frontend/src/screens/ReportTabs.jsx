  ...
  return <div className="report-dashboard">
    ...
    {tab === "insight" && <>
      <section className="dashboard-card manager-daily-summary">
        ...
        <div className="manager-summary-line"><span><b>{summary.calls}</b>통</span><span><b>{Math.round(summary.total_seconds / 60)}</b>분</span><span><b>{careTotal}</b>개 발화 신호</span><span className={unread.length ? "warn" : ""}><b>{unread.length}</b>개 안전 미확인</span><span><b>{summary.repeat_total}</b>회 반복</span></div>
    ...
    {false && tab === "insight" && <>
      <div className="metric-grid">
        <MetricCard label="완료된 AI 통화" value={summary.calls} unit="회" note={`통화 연결 횟수 · 총 ${Math.round(summary.total_seconds / 60)}분`} />
        <MetricCard label="분류된 발화 신호" value={careTotal} unit="건" note="한 통화에서 여러 신호가 관찰될 수 있음" />
        <button className="metric-card heart metric-button" onClick={() => setShowFamilyMoment(true)}><span>가족에게 전할 순간</span><div><b>{summary.meaningful_total || 0}</b><em>건</em></div><small>선별 근거와 전달 요점 보기 ↗</small></button>
      ...
      <section className="dashboard-card rhythm-card rhythm-comparison-card">
        <header className="rhythm-head">
          <div>
            <p className="eyebrow">생활 리듬</p>
            <h2>선택일이 평소와 어떻게 달랐는지 비교합니다</h2>
            <p>같은 달의 일평균을 기준선으로 두고 시간대별 통화량과 발화 유형의 변화를 함께 봅니다.</p>
          </div>
          <span className={`rhythm-ready ${activeBaseline?.rhythm?.ready ? "ready" : ""}`}>
            월 기준선 {activeBaseline?.rhythm?.days_observed || 0}일 · {activeBaseline?.rhythm?.calls_analyzed || 0}통
          </span>
        </header>
        {activeDaySummary && activeBaseline ? <>
          <div className="rhythm-compare-grid"><TimeComparisonChart daySummary={activeDaySummary} baselineSummary={activeBaseline} /><SignalRadar daySummary={activeDaySummary} baselineSummary={activeBaseline} /></div>
          <section className="deviation-insights"><header><div><h3>평소보다 달랐던 발화</h3><p>단순히 많이 나온 항목이 아니라 월 일평균에서 얼마나 벗어났는지 계산했습니다.</p></div><span>{activeDaySummary.since}</span></header><div>{deviations.slice(0, 4).map((item) => <article key={item.signal} className={item.difference > 0 ? "up" : "down"}><div><span>{item.label}</span><b>{item.today}회</b><small>월평균 {item.average.toFixed(1)}회 · {item.difference >= 0 ? "+" : ""}{item.difference.toFixed(1)}</small></div><blockquote>“{item.evidence || "선택일 직접 발화 없음"}”</blockquote><p>{item.action}</p></article>)}</div></section>
          <p className="rhythm-policy">통화량만으로 외로움이나 불안을 단정하지 않습니다. 변화 수치는 실제 발화 분류 결과와 함께 해석하며 진단 점수가 아닙니다.</p>
        </> : <div className="rhythm-waiting"><b>비교 기준을 불러오는 중입니다</b><p>선택일과 같은 달의 데이터가 준비되면 월평균 비교를 보여드립니다.</p></div>}
      </section>
    ...
    {false && tab === "talk" && <>
      <section className="dashboard-card daily-report-overview">
        <header>
          <div><p className="eyebrow">선택 기간 통화 분석</p><label className="report-title-date"><h2>{period.mode === "day" ? `${dateParts(summary.since).date} 리포트` : `${period.value.replace("-", "년 ")}월 리포트`} <span>⌄</span></h2><input type={period.mode === "day" ? "date" : "month"} value={period.value} max={period.mode === "day" ? todayKey : currentMonth} onChange={(event) => event.target.value && onPeriod({ ...period, value: event.target.value })} /></label><p>제목을 눌러 다른 날짜를 선택할 수 있습니다.</p></div>
          <div className="report-mode-switch"><button className={period.mode === "day" ? "on" : ""} onClick={() => onPeriod({ mode: "day", value: todayKey })}>일별</button><button className={period.mode === "month" ? "on" : ""} onClick={() => onPeriod({ mode: "month", value: currentMonth })}>월별</button></div>
        </header>
        <div className="daily-overview-grid">
          <article className="total-call-metric"><span>전체 통화</span><b>{summary.calls}<em>회</em> <i>·</i> {Math.round(summary.total_seconds / 60)}<em>분</em></b><small>완료된 AI 통화의 횟수와 실제 연결 시간</small></article>
        ...
        {period.mode === "day" && <section className="operational-forecast" aria-label="선택한 날의 패턴 기반 주의 예측"><header><div><span>패턴 기반 주의 예측</span><h3>{operationalInsight.title}</h3></div><b>{operationalInsight.basis}</b></header><div className="forecast-grid"><article><small>무엇을 알 수 있나</small><p>{operationalInsight.meaning}</p></article><article><small>다음에 주의할 가능성</small><p>{operationalInsight.next}</p></article><article><small>담당자 선제 행동</small><p>{operationalInsight.action || "추가 행동 근거가 없습니다."}</p></article></div><p className="forecast-policy">진단이나 임상 예측이 아니라, 선택일과 월평균의 통화·발화 차이를 이용한 운영상 주의 신호입니다.</p></section>}
      </section>
