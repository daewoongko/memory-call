...
test("가족이 받아도 24.2초 AI 영상·대기 음악 뒤에 사람 통화를 연다", () => {
  ...
  assert.match(app, /INTRO_COMPLETE_POLL_MS = 250/);
  assert.match(app, /setTimeout\(tick, INTRO_COMPLETE_POLL_MS\)/);
  assert.match(app, /introSecondsLeft <= 2\.5[\s\S]*?nextPollMs = INTRO_COMPLETE_POLL_MS/);
  assert.match(app, /setTimeout\(tick, nextPollMs\)/);
  assert.match(app, /introDurationSec=\{invite\?\.intro_duration_sec \?\? CALLING_INTRO_SEC\}/);
  assert.match(app, /introStartedAt=\{invite\?\.created_at \?\? null\}/);
  assert.match(calling, /introDurationSec = 24\.2/);
  assert.match(calling, /fallbackStartedAtRef = useRef\(Date\.now\(\)\)/);
  assert.match(calling, /const deadline = startedAt \+ durationMs/);
  assert.match(calling, /window\.setInterval\(tick, 250\)/);
  assert.match(calling, /remaining > 0[\s\S]*?"곧 연결돼요"/);
