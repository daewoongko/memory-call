...
서버에는 `ELEVENLABS_API_KEY`가 필요합니다. 가족 설정 화면에서 생성·승인한 가족별 `voice_id`는 SQLite에 저장되며 전역 기본 목소리로 대체하지 않습니다. 원본 녹음은 정적 URL로 공개되지 않는 `data/private_voice/`에 저장되고 Git에서 제외됩니다. 설정 화면의 **목소리 등록 삭제**를 누르면 ElevenLabs 복제 음성과 서버 녹음이 함께 삭제됩니다. **립싱크 없이 순수 음성 통화는 이 설정만으로 완전히 동작하며, 로컬 PC를 켜 둘 필요가 없습니다.**

기존 단일 음성 등록 CLI(`tools/elevenlabs_clone_voice.py`)는 공급자 연결을 확인하는 수동 진단 도구로만 유지합니다. 실제 통화는 설정 화면에서 승인된 가족별 목소리만 사용합니다.

### 가족별 AI 영상 얼굴

가족 설정의 사진 관리에서 현재 기준 사진을 확정하면 서버가 Anam용 정사각형 사진을 만들고 가족별 custom avatar를 생성합니다. 공급자 `avatar_id`와 생성 당시 모델은 SQLite에만 저장되며 브라우저에는 노출되지 않습니다. AI 통화가 시작되면 선택한 가족의 저장된 avatar와 승인된 ElevenLabs voice를 서버가 결합하고, 브라우저에는 해당 통화용 단기 세션 토큰만 전달합니다. 서버 환경변수에는 `ANAM_API_KEY`만 필요하며 전역 `ANAM_AVATAR_ID`는 사용하지 않습니다.
